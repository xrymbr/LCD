...
static void lcd_init_myfb_info(struct display_timing *dt,dma_addr_t *phy_addr)
{
	/* 1.1 分配fb_info */
...
static int lcd_controller_init(struct imx6ull_lcdif *lcdif,struct display_timing *dt,int lcd_bpp,int fb_bpp,unsigned int fb_phy)
{
	...
	*/
   	...
   	lcdif->VDCTRL0 = (1<<28) | (vsync_pol << 27) | (hsync_pol << 26)\
   					|(clk_pol << 25) | (de_pol << 24) | (1 << 21)\
   					|(1 << 21) | (dt->vsync_len.typ << 0);
   ...
   lcdif->VDCTRL1 = dt->vsync_len.typ + dt->vback_porch.typ + dt->vactive.typ + dt->vfront_porch.typ;
   ...
   lcdif->VDCTRL2 = (dt->hsync_len.typ << 18) | (dt->hsync_len.typ + dt->hback_porch.typ + dt->hactive.typ + dt->hfront_porch.typ);
   ...
   lcdif->VDCTRL3 = ((dt->hsync_len.typ + dt->hback_porch.typ) << 16) | (dt->vsync_len.typ + dt->vback_porch.typ);
...
static int mylcd_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		printk("failed to get property bus-width\n");
	...
	if (ret < 0) {
		printk("failed to get property bits_per_pixel\n");
		of_node_put(display_np);
		return ret;
	}	
	...
	/* 设置lcd_init_myfb_info */
	lcd_init_myfb_info(dt,&phy_addr);
	...
	ret = lcd_controller_init(lcdif,dt,bits_per_pixel,16,phy_addr);
	if(ret)
	{
		printk("lcd_controller_init is err\n");
		return -EINVAL;
	}
